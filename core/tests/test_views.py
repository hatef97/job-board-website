...
class ApplicantProfileViewSetTests(APITestCase):
    """Tests for ApplicantProfileViewSet: list, retrieve, create, update, delete & permissions."""
    ...
    def setUp(self):
        ...
        # Give admin an 'applicant' role so they can pass any role checks if needed.
        self.admin_user.role = 'applicant'
        self.admin_user.save()

        self.applicant_user = User.objects.create_user(
            username='applicant', email='applicant@test.com', password='password123'
        )
        self.applicant_user.role = 'applicant'
        self.applicant_user.save()

        self.other_applicant = User.objects.create_user(
            username='other_applicant', email='other@test.com', password='password123'
        )
        self.other_applicant.role = 'applicant'
        self.other_applicant.save()

        self.employer_user = User.objects.create_user(
            username='employer', email='employer@test.com', password='password123'
        )
        self.employer_user.role = 'employer'
        self.employer_user.save()
        ...
        self.profile = ApplicantProfile.objects.create(
            ...
            resume=None,
            bio='I am applicant one'
        )
        self.other_profile = ApplicantProfile.objects.create(
            user=self.other_applicant,
            resume=None,
            bio='I am applicant two'
        )

        # Route names from your router
        self.list_url = reverse('applicant-profiles-list')
        self.detail_url = lambda pk: reverse('applicant-profiles-detail', kwargs={'pk': pk})
    ...
    def test_applicant_can_list_own_profile(self):
        self.authenticate(self.applicant_user)
        res = self.client.get(self.list_url)
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(len(res.data), 1)
        self.assertEqual(res.data[0]['user'], self.applicant_user.pk)
    ...
    def test_applicant_can_retrieve_own_profile(self):
        self.authenticate(self.applicant_user)
        res = self.client.get(self.detail_url(self.profile.pk))
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.assertEqual(res.data['bio'], 'I am applicant one')


    def test_applicant_cannot_retrieve_other(self):
        self.authenticate(self.applicant_user)
        res = self.client.get(self.detail_url(self.other_profile.pk))
        self.assertEqual(res.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_applicant_can_create_profile(self):
        # Remove existing to test create path
        self.profile.delete()
        self.authenticate(self.applicant_user)
        ...
        res = self.client.post(self.list_url, payload, format='json')
        self.assertEqual(res.status_code, status.HTTP_201_CREATED)
        self.assertTrue(ApplicantProfile.objects.filter(user=self.applicant_user).exists())
        self.assertEqual(res.data['bio'], 'New Bio')


    def test_employer_cannot_create_profile(self):
        self.authenticate(self.employer_user)
        payload = {'bio': 'Should not work'}
        res = self.client.post(self.list_url, payload, format='json')
        self.assertEqual(res.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_applicant_cannot_update_other(self):
        self.authenticate(self.applicant_user)
        res = self.client.patch(
            self.detail_url(self.other_profile.pk),
            {'bio': 'Hack Attempt'},
    ...
    def test_admin_can_update_any(self):
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.other_profile.refresh_from_db()
        self.assertEqual(self.other_profile.bio, 'Admin Bio')
    ...
    def test_owner_can_delete_profile(self):
        ...
        res = self.client.delete(self.detail_url(self.profile.pk))
        self.assertEqual(res.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(ApplicantProfile.objects.filter(pk=self.profile.pk).exists())


    def test_applicant_cannot_delete_other(self):
        self.authenticate(self.applicant_user)
        res = self.client.delete(self.detail_url(self.other_profile.pk))
        self.assertEqual(res.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_admin_can_delete_any(self):
        ...
        res = self.client.delete(self.detail_url(self.other_profile.pk))
        self.assertEqual(res.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(ApplicantProfile.objects.filter(pk=self.other_profile.pk).exists())
    ...
    def test_applicant_can_upload_resume_via_update(self):
        self.authenticate(self.applicant_user)
        ...
        self.assertEqual(res.status_code, status.HTTP_200_OK)
        self.profile.refresh_from_db()
        self.assertTrue(bool(self.profile.resume))
